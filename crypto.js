...
export async function deriveKey(password, salt, iterations = ITERATIONS, extractable = false) {
  V.string(password,1,512,'master password',{controls:true});
  V.canonical64(salt,32,32);
  if (iterations !== ITERATIONS || unb64(salt).length !== 32) throw new Error('Unsupported vault key settings.');
  const bytes=enc.encode(password);let material;try{material=await crypto.subtle.importKey('raw',bytes,'PBKDF2',false,['deriveKey']);}finally{bytes.fill(0);}
  return crypto.subtle.deriveKey({name:'PBKDF2',hash:'SHA-256',salt:unb64(salt),iterations}, material, {name:'AES-GCM',length:256}, extractable, ['encrypt','decrypt']);
}
export async function seal(key, value, context) {
  V.string(context,1,256,'encryption context');
  const iv = crypto.getRandomValues(new Uint8Array(12));
  const data = enc.encode(JSON.stringify(value));
  if(data.length>32752){data.fill(0);throw new V.ValidationError('Record is too large.');}
  let ciphertext;try{ciphertext=await crypto.subtle.encrypt({name:'AES-GCM',iv,additionalData:enc.encode(context),tagLength:128},key,data);}finally{data.fill(0);}
  return {v:1,iv:b64(iv),data:b64(ciphertext)};
}
export async function open(key, envelope, context) {
  envelope=V.envelope(envelope);
...
export const importKey = jwk => crypto.subtle.importKey('jwk',V.jwk(jwk),{name:'AES-GCM'},false,['encrypt','decrypt']);
export const itemContext = (uid,id) => `pocket-vault:v1:${V.uuid(uid)}:item:${V.uuid(id)}`;
export const verifierContext = uid => `pocket-vault:v1:${V.uuid(uid)}:verifier`;
