# Acceptance checks for version 1.1
...
7. Lock the vault while a network request is slow. It must stay locked when the response completes. Wait five minutes, then interact with a previously suspended tab: it must lock before showing/copying credentials.
